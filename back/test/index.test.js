...
describe('Test de RUTAS', () => {
    describe('GET /rickandmorty/character/:id', () => {
      // Tu lógica de prueba asincrónica aquí
      ...
      describe("GET /rickandmorty/login",() =>{
        it('Responde segun las credenciales correctas', async ()=>{
            const { body } = await agent.get('/rickandmorty/login?email=edsonnaza@mail.com&password=Abc12345');
            expect(body).toEqual({
                access:true,
            })
        })
        it('Responde segun las credenciales incorrectas', async()=>{
            const { body } = await agent.get(
                '/rickandmorty/login?email=edsonnaza@gmail.com&password=hola2345'
            );
      ...
      describe('DELETE /rickandmorty/fav/:id', ()=>{
        it('Debe devolver el arreglo si o id no se encuentra', async () =>{
